...
function* searchPratsByKey({ key }) {
  console.log("recherche de praticien");
  let url = BASE_URL + "/users/searchBySpeciality/" + key;
  try {
    const result = yield getUnauthRequest(url);
    console.log("Opération réussit ? ", result.success ? "Oui" : "Non");
    if (result.success) {
      console.log("données : ", result.data);
      yield put({
        type: types.SEARCH_PRAT_BY_KEY_SUCCESS,
        payload: result.data,
      });
      // RootNavigation.navigate(SCREENS.HOME_CONTAINER_ROUTE)
    } else {
      yield put({
        type: types.SEARCH_PRAT_BY_KEY_FAILED,
        payload: result.message,
      });
    }
  } catch (error) {
